...
from app.utils import load_schema, log_to_langsmith
...
def generate_stream_response(user_input: str) -> Union[AsyncGenerator, Dict[str, Any]]:
    try:
        if settings.USE_STRUCTURED_OUTPUT:
            response_generator = generate_structured_response_stream(user_input)
            log_to_langsmith(
            ...
            response_generator = generate_unstructured_response_stream(user_input)
            log_to_langsmith(
